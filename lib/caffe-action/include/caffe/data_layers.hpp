...
protected:
	...
	protected:
//        boost::ptr_vector<ImageInfo> total_image_info;
		boost::ptr_vector<ImagePair> total_image_info;
		virtual void InternalThreadEntry();
		int item_id;
		void random_shuffle_totalmetadata(const int batch_size);
//        void read_json(const std::string &img_prefix, const std::string &json_path);
		void read_file(const std::string &file_name, std::string &img_prefix, const int batch_size);
...
public:
	...
	virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
			const vector<Blob<Dtype>*>& top);
	map<std::string, vector<vector<int> > > person_map;
