...
//    if (param_.open_data_layer_control()) {
//      std::ofstream fdata_controller(param_.controller_path().c_str());
//      if (fdata_controller.is_open()) {
//        int dataset_size = param_.dataset_size();
//        int line_id = rand() % dataset_size;
//        std::stringstream ss;
//        ss << line_id;
//        fdata_controller << ss.str();
//        fdata_controller.close();
//        ss.clear();
//      } else {
//        LOG(ERROR) << "File " << param_.controller_path() << " cannot be visited!";
//      }
//    }
...
//      LOG(INFO) << "average_loss: " << average_loss;
//      LOG(INFO) << "losses[" << idx << "]: " << losses[idx];
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  // overridden by setting snapshot_after_train := false
    Dtype cycle_size = ceil(Dtype(this->param_.max_iter()) / Dtype(this->param_.num_cycle()));
    int iter_cycle = int(iter_ - 1) % int(cycle_size);
  if ((param_.snapshot_after_train()
      && (!param_.snapshot() || iter_ % param_.snapshot() != 0)) || iter_cycle == 0) {
...
Dtype SGDSolver<Dtype>::GetLearningRate() {
  ...
  if (lr_policy == "fixed") {
    ...
    rate = this->param_.base_lr()
      ...
      Dtype cycle_size = ceil(Dtype(this->param_.max_iter()) / Dtype(this->param_.num_cycle()));
      int iter_cycle = int(this->iter_ - 1) % int(cycle_size);

      rate = this->param_.base_lr() * Dtype(0.5) * (cos(Dtype(PI) * iter_cycle / cycle_size) + 1);
      if (iter_cycle == 0){
          LOG(INFO) << "Iteration " << this->iter_ << " reset lr to " << rate;
      }
  }
    else {
...
//      LOG(INFO) << "diff: " << net_params[i]->sumsq_diff();
    }
  }
...
//      LOG(INFO) << "DIFF: "<<net_params[param_id]->cpu_diff()[i];
//    }
    caffe_copy(net_params[param_id]->count(),
