...
__global__ void ScaleBiasForward(const int n, const Dtype* in,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    template<typename Dtype>
    void visualize_scale(const Dtype *data, int width, int height, string img_key) {
      cv::Mat img1 = cv::Mat::zeros(cv::Size(width, height), CV_8UC1);
      for (int y = 0; y < height; y++)
        for (int x = 0; x < width; x++) {
          unsigned char &rgb = img1.at<uchar>(y, x);
          rgb = static_cast<unsigned char>(data[y * width + x] >= 107.5 && data[y * width + x] <= 147.5? 0 : 255);
        }
...
//		cv::convertScaleAbs(img1, output);
      cv::imwrite("visualize/" + img_key + ".jpg", img1);
...
ss_input << rand_id << "_" << top[0]->shape(3) << "_" << top[0]->shape(2) << "img_" << n << "_" << c << "_scale";
const int img_pos = n * top[0]->shape(3) * top[0]->shape(2) * top[0]->shape(1);
const int ch_pos = c * top[0]->shape(3) * top[0]->shape(2);
visualize_scale(top[0]->cpu_data() + img_pos + ch_pos, top[0]->shape(3), top[0]->shape(2), ss_input.str());
